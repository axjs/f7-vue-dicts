...
<f7-page>
    ...
    <f7-navbar back-link="Back" :title="'Dict '+dict" sliding></f7-navbar>
    <!-- <f7-block-title co>List</f7-block-title> -->
    <f7-list contacts>
        <f7-list-item swipeout :link="/edit/+dict+'/'+item['.key']" :key="item['.key']" v-for="item in items" :title="item.name" :badge="item['.key']" badge-color="red">
            <f7-swipeout-actions>
...
export default {
    ...
    beforeMount: function() {
        this.dict = this.$route.params.name
    }
}
</script>
